...
// ── DROPBOX ──
async function dbx(ep, body) {
  var r = await fetch('https://api.dropboxapi.com/2/'+ep, {
    method:'POST',
...
async function getLink(path) {
  var r = await fetch('https://api.dropboxapi.com/2/files/get_temporary_link', {
    method:'POST',
    headers:{'Authorization':'Bearer '+DBX,'Content-Type':'application/json'},
...
async function getThumb(path) {
  ...
  try {
    var r = await fetch('https://content.dropboxapi.com/2/files/get_thumbnail_v2', {
      method:'POST',
      headers:{
        'Authorization':'Bearer '+DBX,
        'Dropbox-API-Arg':JSON.stringify({resource:{'.tag':'path',path:path},format:{'.tag':'jpeg'},size:{'.tag':'w640h480'},mode:{'.tag':'fitone_bestfit'}}),
        'Content-Type':''
      }
    })
    if (!r.ok) return null
...
async function lsFolder(path) {
  try {
    var d = await dbx('files/list_folder', {path:path,recursive:false,include_media_info:true})
    return d.entries || []
...
function injectStyles() {
  ...
  s.textContent = [
    ...
    '.top-title{font-size:13px;font-weight:600;color:#1a1a2e}',
    ...
    // athlete card
    '.athlete-card{display:flex;align-items:center;gap:14px;padding:16px;border-bottom:1px solid #f0f0f0;background:#fafafa}',
    '.athlete-photo{width:56px;height:56px;border-radius:50%;object-fit:cover;border:2px solid #e8e8e8;flex-shrink:0}',
    '.athlete-photo-placeholder{width:56px;height:56px;border-radius:50%;background:#e8f0ff;display:flex;align-items:center;justify-content:center;flex-shrink:0;font-size:20px;font-weight:600;color:#0061ff}',
    '.athlete-info{}',
    '.athlete-title{font-size:11px;color:#aaa;text-transform:uppercase;letter-spacing:0.5px;margin-bottom:2px}',
    '.athlete-name{font-size:17px;font-weight:600;color:#1a1a2e}',
    // breadcrumb
    ...
    '.auth-box{width:100%;max-width:340px;background:#fff;border-radius:12px;padding:32px 28px;box-shadow:0 2px 16px rgba(0,0,0,0.07)}',
    '.auth-hd{text-align:center;margin-bottom:22px}',
    '.auth-hd img{width:42px;height:42px;border-radius:9px;display:block;margin:0 auto 10px}',
    '.auth-hd h1{font-size:16px;font-weight:600;color:#1a1a2e}',
    '.auth-hd p{font-size:12px;color:#aaa;margin-top:2px}',
    ...
    '.auth-sub{width:100%;padding:11px;background:#0061ff;color:#fff;border:none;border-radius:7px;font-size:14px;font-weight:600;cursor:pointer;margin-top:2px}',
